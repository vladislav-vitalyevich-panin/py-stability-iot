...
def trigger_rtc(rtc_path, rtc_mode):
    return os.system('rtcwake --device ' + rtc_path + ' -s 60 -m ' + rtc_mode)

def trigger_wdt(indicator_file, wdt_path, wdt_kill_delay):
    """
...
def handle_rtc_reboot(config, indicator_file):
...
def handle_wdt_reboot(config, indicator_file):
...
def monitor_system_continuous_wdt(config):
    indicator_file = config.get('general','indicator_file_path').rstrip()
...
def monitor_system_continuous_rtc(config):
    indicator_file = config.get('general','indicator_file_path').rstrip()
    checking_interval = int(config.get('general','checking_interval'))
    while(True):
        if os.path.exists(indicator_file):
